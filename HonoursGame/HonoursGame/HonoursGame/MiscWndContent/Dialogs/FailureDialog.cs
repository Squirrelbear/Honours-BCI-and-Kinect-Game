...
namespace HonoursGame
{
    class FailureDialog : WndHandle
    {
        ...
        public FailureDialog(String message, WndHandle.WndType nextWnd, bool allowretry, int wndWidth, int wndHeight, Game1 appRef)
            : base(WndType.FailureDialog, wndWidth, wndHeight, appRef)
        {
            ...
            if(allowretry)
                btnSpriteOffs.Add(appRef.Content.Load<Texture2D>("OtherDialog\\tryagainout"));
            else
            ...
            if (allowretry)
                btnSpriteOns.Add(appRef.Content.Load<Texture2D>("OtherDialog\\tryagainover"));
            else
